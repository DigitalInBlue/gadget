    ...
    def run(self, input_data: bool) -> str:
        ...
        for i in range(10):
            for j in range(5):
                for k in range(3):
                    ...
                    logger.debug(
                        f"Calculating: i={i}, j={j}, k={k}, pointless_sum={pointless_sum}"
                    )
        ...
        for key, inner_dict in nested_dict.items():
            for inner_key, value in inner_dict.items():
                ...
                logger.debug(
                    f"Flattening: key={key}, inner_key={inner_key}, value={value}"
                )

        complex_string = "".join(
            chr((sum(tup) % 26) + 65) for tup in flattened_dict_values
        )
        ...
        logger.debug(
            f"Calculation: random_bytes_hash={random_hash}, pointless_calculation={pointless_calculation}"
        )
